...
async function getCachedRouteDetails(gtfsId) {
...
async function cacheRouteDetails(gtfsId, routeData) {
    try {
        // Cache in Redis
...
async function fetchRouteDetailsWithCache(gtfsId) {
    ...
    // Fetch from API
    try {
        const routeData = await mav_1.mavApi.getTrainDetails(gtfsId);
        if (routeData) {
...
async function runFetchCycle() {
    ...
    try {
        ...
        const trains = await Promise.all(validTrains.map(async (mavTrain) => {
            ...
            return (0, transformers_1.transformMavTrain)(mavTrain, routeData);
        }));
        ...
        if (trainsWithRoute.length > 0) {
            console.log('🚂 Sample routes detected:', trainsWithRoute.slice(0, 3).map(t => {
                ...
                return ({
                    number: t.number,
                    route: `${(_a = t.origin) === null || _a === void 0 ? void 0 : _a.name} -> ${(_b = t.destination) === null || _b === void 0 ? void 0 : _b.name}`
                });
            }));
        }
        if (trains.length === 0) {
